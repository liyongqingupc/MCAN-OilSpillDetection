...
def SinGAN_generate(Gs,Zs,real1s,real2s,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=50): #reals -- real1s,real2s
    #if torch.is_tensor(in_s) == False:
    if in_s is None:
        in_s = torch.full(real2s[0].shape, 0, device=opt.device)
    images_cur = []
