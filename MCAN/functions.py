...
def calc_gradient_penalty(netD, input_data, real_data, fake_data, LAMBDA, device):
    ...
    #disc_interpolates = netD(interpolates)  # lyq # 0911
    disc_interpolates = netD(torch.cat((input_data, interpolates),1))   # lyq add torch.cat
...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        dir2save = 'TrainedModels/%s' % 'model'
    return dir2save
