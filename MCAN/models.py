...
class ConvBlock(nn.Sequential):   # nn.Sequential: father class
    def __init__(self, in_channel, out_channel, ker_size, padd, stride): #padd: add 0
        super(ConvBlock,self).__init__()
        self.add_module('conv',nn.Conv2d(in_channel ,out_channel,kernel_size=ker_size,stride=stride,padding=padd)),
        self.add_module('norm',nn.BatchNorm2d(out_channel)),
        self.add_module('LeakyRelu',nn.LeakyReLU(0.2, inplace=True))  # LeakyRelu:improved Relu
...
class WDiscriminator(nn.Module):  # W
    def __init__(self, opt):
        ...
        for i in range(opt.num_layer-2):
            N = int(opt.nfc/pow(2,(i+1))) # pow: mi
